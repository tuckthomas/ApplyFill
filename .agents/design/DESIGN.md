...
- `ProfileResumeImportSection.tsx`: pre-personal-info PDF/DOCX/TXT import, plain-language Private AI status, selectable proposal review, retained-source metadata, and source preview. An existing profile opens a themed Merge, Replace, or Cancel decision; merge review lets users choose detected contact fields that replace saved values.
