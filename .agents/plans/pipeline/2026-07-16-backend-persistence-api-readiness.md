...
**Status:** Pipeline — review and approval required before work begins  
**Created:** 2026-07-16  
...
Today, the profile builder, job tracker, dashboard widgets/layouts, and preferences save in browser `localStorage`. The only form-adjacent API interactions are profile-consent capture and AI text enhancement. There are no resource controllers, request/response contracts, application services, ownership checks, concurrency rules, migration path for browser data, or integration tests for the product data.
...
- [ ] Review the profile-versus-resume ownership decision.
- [ ] Review the dedicated `JobApplication` aggregate recommendation.
- [ ] Approve Phase 0 only.
- [ ] Move this document from `pipeline/` to `in-progress/` after explicit instruction to begin.
- [ ] Do not start Phase 1 until Phase 0 exit criteria are met and reviewed.
...
- [ ] Define API DTOs in the Application layer or a dedicated contracts project, with explicit mapping.
- [ ] Decide and document canonical profile versus default-resume ownership.
- [ ] Add database entities and migrations for alternative names and application-question answers.
- [ ] Define the server-side restricted rich-text JSON validation policy.
- [ ] Add a dedicated job-application aggregate and migration; do not force the tracker into `JobTarget` and `ApplicationLog`.
- [ ] Reconcile UI-only fields with domain entities; remove or deliberately defer unsupported fields only with product approval.
- [ ] Add an authenticated-user abstraction so controllers do not parse claims ad hoc.
- [ ] Standardize `ProblemDetails`, validation, logging, and exception handling.
- [ ] Define optimistic concurrency and collection replacement/merge rules.
- [ ] Add API integration-test infrastructure using an isolated PostgreSQL test database or Testcontainers.
...
- [ ] Ownership, validation, and concurrency behavior have integration-test coverage.
- [ ] Confirm no controller exposes EF entities.
- [ ] Review Phase 0 outcome before authorizing Phase 1.

### Phase 1 — Profile aggregate persistence

**Goal:** Persist the entire profile builder safely as one server-backed aggregate.

- [ ] Implement profile query and transactional update service/controller.
- [ ] Persist address, alternative names, web links, application questions, and approved profile fields.
- [ ] Validate and store only restricted rich-text JSON where the approved schema contains rich text.
- [ ] Keep consent recording separate; add consent status to a profile-read projection only if the UI needs it.
- [ ] Add frontend API client, loading/error/retry states, and a one-time import from the existing local profile key only if explicitly approved.
...
- [ ] Verify profile reload in a fresh browser session.
- [ ] Verify the same profile is available from a second browser session.
- [ ] Verify invalid and conflicting writes preserve existing data.
- [ ] Verify owner isolation and aggregate updates with integration tests.
...
- [ ] Verify every resource query filters by the authenticated user.
- [ ] Keep the Gemini key and all provider calls server-side.
- [ ] Validate request payloads at the API boundary, including nested collections and rich-text JSON allow-lists.
- [ ] Do not render or migrate raw HTML; retain the structured-document-only policy.
...
- [ ] Back up PostgreSQL and test restore/migration procedures before user data becomes authoritative.

## Verification checklist

- [ ] Add unit tests for mappings, validation, state transitions, and rich-text schema validation.
- [ ] Add API integration tests for authorization/ownership isolation and CRUD.
- [ ] Add API integration tests for validation failures, concurrency conflicts, and transaction rollback.
- [ ] Test migrations against a clean PostgreSQL instance.
...
- [ ] Keep this plan in `pipeline/` while it is under review.
- [ ] On explicit approval and instruction to execute, move this file unchanged to `in-progress/` and begin only Phase 0.
