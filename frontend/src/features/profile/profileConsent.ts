...
export const PROFILE_AUTOMATION_DISCLOSURES = {
  ...
  localData: 'Your profile, resumes, job tracker, dashboard, and Browser Agent history are stored in ApplyFill\'s PostgreSQL database on this computer. ApplyFill does not send them to an ApplyFill cloud account. Anyone with access to your signed-in computer may still be able to access local applications, so protect the device and keep backups.',
...
export const PROFILE_AUTOMATION_CONSENT_ACKNOWLEDGMENT =
  'I understand that my data is stored by ApplyFill in a local PostgreSQL database on this computer, is not backed up to an ApplyFill cloud account, and may appear in files or clipboard copies I intentionally export. I understand that Private AI runs on this computer, receives only the information required for the action, and requires my review before profile or resume changes and before final application submission.';
