...
const deepFreeze = (value: unknown): Readonly<Record<string, unknown>> => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('Invalid local AI tool snapshot.');
  freezeNested(value);
  return value as Readonly<Record<string, unknown>>;
};

export const validateAiToolCall = (value: unknown, allowedOpaqueIds: Set<string>): AiToolCall => {
  if (!value || typeof value !== 'object' || Array.isArray(value)) throw new Error('Invalid local AI tool call.');
  ...
  if (!allowedKeys(call, ['arguments', 'name', 'schemaVersion']) || call.schemaVersion !== AI_TOOL_SCHEMA_VERSION
    ...
    throw new Error('Unknown or invalid local AI tool call.');
  }
  const args = call.arguments as Record<string, unknown>;
  if (!allowedKeys(args, ['opaqueIds', 'sourceOpaqueId', 'text'])) throw new Error('Invalid local AI tool arguments.');
  if ('sourceOpaqueId' in args && (typeof args.sourceOpaqueId !== 'string' || !allowedOpaqueIds.has(args.sourceOpaqueId))) {
    throw new Error('Local AI referenced content outside the approved snapshot.');
  }
  if ('opaqueIds' in args && (!Array.isArray(args.opaqueIds) || args.opaqueIds.length > 100
    ...
    throw new Error('Local AI requested content outside the approved snapshot.');
  }
  if ('text' in args && (typeof args.text !== 'string' || args.text.length > 2_000)) throw new Error('Local AI tool text is invalid.');
...
export const executeAiToolCalls = async <Result>(
  ...
  if (calls.length > AI_TOOL_LIMITS.maximumCalls) throw new Error('Local AI requested too many tool calls.');
  ...
  for (const value of calls) {
    if (combined.aborted) throw new DOMException('Local AI tool execution was cancelled.', 'AbortError');
